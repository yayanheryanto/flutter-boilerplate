// ─────────────────────────────────────────────────────────────────────────────
// DESIGN SYSTEM BARREL EXPORT
// Import this single file to access all shared UI components:
//   import 'package:boilerplate/shared/widgets/design_system.dart';
// ─────────────────────────────────────────────────────────────────────────────

// ── TYPOGRAPHY ────────────────────────────────────────────────────────────────
export 'typography/app_text.dart';

// ── BUTTONS ───────────────────────────────────────────────────────────────────
export 'buttons/app_button.dart';

// ── INPUT ─────────────────────────────────────────────────────────────────────
export 'input/app_text_field.dart';
export 'input/app_form_inputs.dart';

// ── DISPLAY ───────────────────────────────────────────────────────────────────
export 'display/app_display.dart';
export 'display/app_avatar.dart';
export 'display/app_badge.dart';
export 'display/app_card.dart';
export 'display/app_chip.dart';
export 'display/app_divider.dart';
export 'display/app_image.dart';
export 'display/app_logo.dart';
export 'display/app_spacer.dart';

// ── FEEDBACK ──────────────────────────────────────────────────────────────────
export 'feedback/app_feedback.dart';

// ── NAVIGATION ────────────────────────────────────────────────────────────────
export 'navigation/app_navigation.dart';

// ── MOLECULES: GENERAL ────────────────────────────────────────────────────────
export 'app_molecules.dart';

// ── MOLECULES: DIALOGS ────────────────────────────────────────────────────────
export 'dialogs/app_dialog.dart';

// ── MOLECULES: BOTTOM SHEETS ──────────────────────────────────────────────────
export 'bottomsheets/app_bottom_sheet.dart';

// ── MOLECULES: TOAST ──────────────────────────────────────────────────────────
export 'toast/app_toast.dart';

// ── MOLECULES: SNACKBAR ───────────────────────────────────────────────────────
export 'snackbar/app_snackbar.dart';

// ── MOLECULES: PICKERS ────────────────────────────────────────────────────────
export 'pickers/app_date_picker.dart';
export 'pickers/app_pickers.dart';

// ── MOLECULES: OVERLAYS ───────────────────────────────────────────────────────
export 'overlays/app_overlays.dart';

// ── SKELETON ──────────────────────────────────────────────────────────────────
export 'skeleton/skeleton_atoms.dart';
export 'skeleton/skeleton_molecules.dart';
export 'skeleton/skeleton_organisms.dart';
